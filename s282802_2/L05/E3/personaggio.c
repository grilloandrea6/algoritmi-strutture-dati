...
void stampaEquipaggiamento(tabPg_t *tabella) {
	...
	for(int i = 0; i < nodo->val.equip.inUso; i++) {
		DEBUG("nome = %s, tipo = %s\n",(nodo->val.equip.vettEq[i])->nome,(nodo->val.equip.vettEq[i])->nome + MAXL*sizeof(char));
		printOggetto(nodo->val.equip.vettEq[i]);
	}
...
void rimuoviOggetto(tabPg_t *tabella) {
	...
	DEBUG("nodo->val.equip.inUso = %d, i = %d\n",nodo->val.equip.inUso,i);

	if(i == nodo->val.equip.inUso && strcmp(str,nodo->val.equip.vettEq[i - 1]->nome) != 0) {
...
void freeTabella(tabPg_t *tabella) {
	nodoPg_t *nodo,*p;
	for(nodo = tabella->head; nodo != NULL; nodo = p) {
		p = nodo->next;
		free(nodo);
	}
	free(tabella);
